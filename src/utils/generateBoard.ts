...
export const generateBoard = (
  width = 40,
  height = 20,
  staringNumberOfPreys = 0.1,
  staringNumberOfPredators = 0.02,

) => {
  const board = Array.from(Array(height), () => Array(width).fill({ type: CellType.Empty }))
  const emptyCellsMap = getEmptyCellsMap(board)
  const preys = []
  const predators = []
  const preysNumber = width * height * staringNumberOfPreys
  const predatorsNumber = width * height * staringNumberOfPredators
